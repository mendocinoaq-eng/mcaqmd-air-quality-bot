"""
MCAQMD Ozone Air Quality Facebook Poster (via Buffer)
Pulls Ozone data from AirNow API and posts to the MCAQMD
...
# ── AQI helpers (ozone-specific messaging) ────────────────────────────────────
...
def aqi_health_tip(aqi):
    ...
    if aqi <= 150:
        return "Children, the elderly, and those with lung conditions (such as asthma) should limit prolonged outdoor exertion, especially in the afternoon when ozone peaks."
    if aqi <= 200:
        return "Everyone should reduce prolonged outdoor exertion. People with lung conditions should avoid outdoor activity. Ozone levels are highest in the afternoon — plan activities for the morning if possible."
    if aqi <= 300:
        return "Everyone should avoid prolonged outdoor exertion. People with lung conditions should remain indoors and keep windows closed."
    return "Health alert: Everyone should avoid all outdoor exertion. Keep windows closed and run air conditioning if available."
...
def build_message(data):
    ...
    ozone = next((d for d in data if d.get("ParameterName") == "O3"), None)

    if not ozone:
        print("No ozone data found in AirNow response — skipping post.")
        return None

    aqi       = ozone["AQI"]
    category  = aqi_category(aqi)
    tip       = aqi_health_tip(aqi)
    area      = ozone.get("ReportingArea", "Mendocino County")

    return (
        f"🌤️ {time_of_day} Ozone Update — {date_str}\n\n"
        f"📍 {area}\n"
        f"🫁 Ozone (O₃) AQI: {aqi} — {category}\n\n"
        f"ℹ️ {tip}\n\n"
        f"For real-time conditions visit airnow.gov or call (707) 463-4354.\n"
        f"🔥 Burn Day Status Line: (707) 463-4391\n\n"
        f"#MendocinoCounty #AirQuality #Ozone #MCAQMD #CleanAir #Ukiah"
    )
...
def main():
    ...
    if message is None:
        print("No ozone data available today — no post made.")
        return
